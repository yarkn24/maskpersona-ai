...
- **Voice isolation:** `pip install "persona-forge[voice]"`. Models download automatically on first run.
- **Exa web search:** `pip install "persona-forge[search]"` and add `EXA_API_KEY=...` to `.env`.
