# FORGE_INSTALL: the agent-executable install

This is the single, ordered install an AI agent can follow to stand up persona-forge. It runs the SAME
...
2. **Dependencies.** Install the pinned deps from `pyproject.toml` into a venv.
   - Voice isolation: `pip install "persona-forge[voice]"` (models download automatically on first run, no token needed).
   - Web search via Exa: `pip install "persona-forge[search]"` then add `EXA_API_KEY=...` to `.env`.
