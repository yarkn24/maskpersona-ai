# Tasks: persona-forge (Specs verifiable task graph)
...
+ zero-trace grep + tests). Status: `[ ]` todo, `[~]` in progress, `[x]` verified.
...
- [ ] **T01 Config layer** (deps: T00) -> `config/persona.schema.yaml`, `persona.example.yaml`, `defaults.yaml`
...
- [ ] **GATE 0:** constitution check + `python -m audit.checks.grep_traces` (none yet, but harness runs) + `pytest tests/test_config.py tests/test_brain.py` green.
...
- [ ] **T05 Hidden injection layer** (deps: T01) -> `agents/_injection/preamble.md`, `inject.py`, `matrix.yaml`
  - `verify:` `inject.py` prepends the preamble to a sample agent prompt; `matrix.yaml` selects per-agent extras; unit test asserts the four mechanisms are present.
...
- [ ] **GATE 1:** constitution + zero-trace grep + `pytest tests/test_templates.py tests/test_injection.py tests/test_citation.py` green.

## Phase 2: Build pipeline

- [ ] **T07 LangGraph pipeline** (deps: T01, T03, T06) -> `pipeline/state.py`, `nodes/*`, `interrupts.py`, `checkpoint.py`, `graph.py`, `run_ingest.py`
  ...
  - `done:` discover -> assess_breadth -> scope/volume gates -> fingerprint -> isolate -> harvest -> write -> mine -> cite, with checkpointing and human interrupts.
  - `rollback:` delete pipeline/; nothing downstream is built yet.

- [ ] **GATE 2:** constitution + zero-trace + `pytest tests/test_pipeline.py` green.
...
- [ ] **GATE 3:** constitution + zero-trace + `pytest tests/test_eval.py tests/test_auditor.py` green.

## Phase 4: Install and docs

- [ ] **T10 Deterministic installer** (deps: all above) -> `installer/bootstrap.py`, `steps/*`, `helpers/*`, `lock/parts.lock.json`, `FORGE_INSTALL.md`
  ...
  - `done:` one master installer (human `make` + agent `FORGE_INSTALL.md`) producing identical output.
  - `rollback:` delete installer/ + FORGE_INSTALL.md.

- [ ] **T11 Docs + CLAUDE.md** (deps: all above) -> `CLAUDE.md`, `docs/*`
  - `verify:` CLAUDE.md present and self-consistent; docs reference only real repo files; zero-trace clean.
  - `done:` architecture/pipeline/voice/patterns/onboarding/legal docs + the project CLAUDE.md.
  - `rollback:` delete docs/ + CLAUDE.md.

- [ ] **GATE 4:** constitution + zero-trace + docs link-check.

## Phase 5: Release audit (the gate that ships)

- [ ] **T12 Audit swarm** (deps: all) -> `audit/run_audits.py`, `genericity_audit.md`, `gdpr_audit.md`, `text_classifier_audit.md`, `legal_lawyer_agent.md`, `checks/grep_traces.py`
